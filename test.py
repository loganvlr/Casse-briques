...
class Balle:
    ...
    def mouvements(self, plateforme):
        if not plateforme.bougeVersGauche and not plateforme.bougeVersDroite:
            self.x += math.sin(self.angle) * self.vitesse * -1
            self.y -= math.cos(self.angle) * self.vitesse * -1

        elif plateforme.bougeVersGauche:
            self.x += math.sin(self.angle) * self.vitesse * -1 - 60
            self.y -= math.cos(self.angle) * self.vitesse * -1
...
class Niveau:
    ...
    def collisionPlateforme(self, balle, plateforme):
        if balle.x > plateforme.x and balle.x < plateforme.x + plateforme.largeur and balle.y > plateforme.y and balle.y < plateforme.y + plateforme.hauteur:
            balle.rebond("vertical")
    
    def collisionMur(self, balle):
        if balle.x <= 0 or balle.x + balle.diametre >= jeu.largeur:  # Si la balle a frappé un mur à gauche ou à droite
            balle.rebond("horizontal")
        if balle.y <= 0 or balle.y + balle.diametre >= jeu.hauteur:  # Si la balle a frappé un mur en haut ou en bas
            balle.rebond("vertical")
    
    def collisionBrique(self, balle):
        for brique in self.briques:
            if balle.x <= brique.x + brique.largeur and balle.x + balle.diametre >= brique.x and balle.y <= brique.y + brique.hauteur and balle.y + balle.diametre >= brique.y:
                balle.rebond("vertical")
                if brique.vie == 1:
            ...
            elif balle.y <= brique.y + brique.hauteur and balle.y + balle.diametre >= brique.y and balle.x <= brique.x + brique.largeur and balle.x + balle.diametre >= brique.x:
                balle.rebond("horizontal")
                self.briques.remove(brique)
                if brique.vie == 1:
                    self.briques.remove(brique)
    ...
    def draw(self, screen):
        ...
        self.balle.mouvements(self.plateforme)
